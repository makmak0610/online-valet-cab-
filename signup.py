...
def connect_database():
    if emailEntry.get() == '' or phoneEntry.get() == '' or usernameEntry.get() == '' or passwordEntry.get() == '' or confirmEntry.get() == '':
        ...
        messagebox.showinfo('error', ' invaild no ')
        ...
        try:
            ...
            query = 'use userdata'
            mycursor.execute(query)
            query = 'create table data(id int auto_increment primary key not null,email varchar(35), phone varchar(10) , username varchar(20), password varchar(20))'
            mycursor.execute(query)
        except:
        ...
        if row != None:
            ...
            query = 'insert into data(email,phone,username,password) values(%s,%s,%s,%s)'
...
signup_window = Tk()
signup_window.configure(background='violet')
signup_window.geometry('1300x600')
